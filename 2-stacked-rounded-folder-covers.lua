...
-- Start position: exact gap above cover
local current_y = image_y - COVER_GAP - LINE_HEIGHT

for i = 1, #LINE_WIDTHS do
    local width = LINE_WIDTHS[i]
    local line_x = center_x - math.floor(width / 2)
    local line_y = current_y

    bb:paintRect(line_x, line_y, width, LINE_HEIGHT, LINE_COLOR)

    -- Move upward for next line
    current_y = line_y - LINE_HEIGHT - LINE_SPACING
end
